...
- **`School`** inherits from `Property` and relates to `Apartment` via **many-to-many unidirectional aggregation** (`Apartment` is the owner side). JPA auto-generates the join table without an explicit intermediate entity.
- **`PropertyContract`** is the **explicit join entity** that resolves the many-to-many relationship between `Owner` and `Property`. Unlike the Apartment→School relation, the join table here is a domain entity with its own attributes (value, percentage, date), so JPA maps it through this class rather than automatically.
- **`Person`** is abstract: it cannot be instantiated directly. `Owner` and `Reviewer` are its only specializations in V1.
- **`Property`** is abstract: the only concrete subtype in V1 is `Apartment`. Subtypes such as `House`, `Duplex` and `Townhouse` will be added in later versions.
- **`Review`** is owned by both `Property` (physical composition) and `Reviewer` (authorial composition). Both are valid: a `Review` cannot exist without its property or its author.
- The **interfaces** (`Registrable`, `Notifiable`, `Followable`) and classes `SingleFamilyHome`, `House`, `Duplex`, `Townhouse` and `Analytics` are deferred to later versions.
- The **logical association** `Owner -- Property` is kept as a conceptual reference; actual persistence is managed exclusively through `PropertyContract`.
